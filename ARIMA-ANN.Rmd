...
# Prepare our Lynx Data

```{r}
data <- read.csv("C:/Users/Antoine Jarry/Desktop/Data/temporal_df.csv")
...
ts_data <- ts(data$Range_area, frequency = 1)
plot(ts_data) # Probably no season
```
...
train_sequence <- normalized_ann[1:45]
val_sequence<- normalized_ann[46:56] #Split our train_data in valditaion_data for our model

train0_dataset <- timeseries_dataset_from_array(
  data = train0_sequence,
  targets = tail(train0_sequence,-2),
  sequence_length = 2, # the past 2 timesteps to predict the next timestep
  batch_size = 7 # size of all our batch
)

train_dataset <- timeseries_dataset_from_array(
  data =  train_sequence,
  targets = tail(train_sequence,-2),
  sequence_length = 2,
  batch_size = 7
)
...
create_and_train_model <- function(num_neurons) {
  inputs <- layer_input(shape = c(2, 1))
  outputs <- inputs %>%
    layer_flatten()%>%
...
plot(train_data, main = "comparing observed and predicted data",
...
rmse <- sqrt(mean((tail(train_data,-2) - pred_ann)^2))
cat("Root Mean Squared Error (RMSE):", rmse, "\n")
```
...
train_sequence <- normalized_multi[1:45]
val_sequence<- normalized_multi[46:56]
...
# Calculer l'erreur de prévision, par exemple, la racine carrée de la moyenne des carrés des écarts
rmse <- sqrt(mean((tail(no_linear_multi,-2) - pred_ann_multi)^2))
cat("Root Mean Squared Error (RMSE):", rmse, "\n")
```
...
train_sequence <- normalized_add[1:45]
val_sequence<- normalized_add[46:56]
...
rmse <- sqrt(mean((tail(no_linear_add,-2) - pred_ann_add)^2))
cat("Root Mean Squared Error (RMSE):", rmse, "\n")
```
...
```{r}
pred_add_final<-tail(predictions_arima,-2)+pred_ann_add
pred_multi_final<-tail(predictions_arima,-2)*pred_ann_multi
...
# MSE add model
mse_1 <- mean((tail(train_data,-2) - pred_add_final)^2)
cat(" Mean Squared Error for additive model:", mse_1, "\n")

# MSE arima model
mse_2 <- mean((tail(train_data,-2) - tail(predictions_arima,-2))^2)
cat(" Mean Squared Error for arima :", mse_2, "\n")

# MSE multi model
mse_3 <- mean((tail(train_data,-2) - pred_multi_final)^2)
cat(" Mean Squared Error for multiplicative model:", mse_3, "\n")

#MSE ANN model 
mse_4<-mean((tail(train_data,-2)-pred_ann)^2)
cat("Mean Squared Error for ann model:", mse_4, "\n")

```
We can see our two hybrids models are behind ARIMA, ANN is far away behind three others.
...
test_add_final<-test_add_final0+tail(test_arima,-2)
...
test_multi_final<-test_multi_final0*tail(test_arima,-2)
```

### Compare our models on test_data

```{r}

# Plot for Additive Model
plot(tail(test_data,-2), type = "l", col = "blue", lty = 1, xlab = "Time", ylab = "Values", main = "Additive Model",ylim = range(c(test_data,test_add_final)))
...
plot(tail(test_data,-2), type = "l", col = "blue", lty = 1, xlab = "Time", ylab = "Values", main = "ARIMA Model")
...
plot(tail(test_data,-2), type = "l", col = "blue", lty = 1, xlab = "Time", ylab = "Values", main = "Multiplicative Model")
lines(data.frame(test_multi_final), col = "red", lty = 1)
legend("topright", legend = c("Test Data", "Multiplicative Model"), col = c(1, 4), lty = 1)

# Plot for ANN Model
plot(tail(test_data,-2), type = "l", col = "blue", lty = 1, xlab = "Time", ylab = "Values", main = "ANN Model")
...
# MSE add model
mse_1 <- mean((tail(test_data,-2) - test_add_final)^2)
cat(" Mean Squared Error for additive model:", mse_1, "\n")

# MSE arima model
mse_2 <- mean((tail(test_data,-2) - tail(test_arima,-2))^2)
cat(" Mean Squared Error for arima :", mse_2, "\n")

# MSE multi model
mse_3 <- mean((tail(test_data,-2) - test_multi_final)^2)
cat(" Mean Squared Error for multiplicative model:", mse_3, "\n")

#MSE ANN model 
mse_4<-mean((tail(test_data,-2)-test_ann)^2)
cat("Mean Squared Error for ann model:", mse_4, "\n")
```

Finally with our test_data we can see ANN model is the best, following by ARIMA and additive model. Far away behind the multiplicative model
Why ? Explain the reason for the difference 
...
for (i in 1:num_future_points) {
  ...
  current_prediction <- predict(best_model, newdata = as.matrix(last_known_data))
